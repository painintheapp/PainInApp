...
import EditModal from './components/EditExercise'
...
const Exercise = () => {
    ...
    const columns = [
        {
            title: 'Exercise ID',
            render: (record) => <span>{record.exerciseId && record.exerciseId}</span>,
            key: "categoryNo",
        },
        {
            title: 'Category',
            ...
            <Widget
                ...
                extra={
                    <div className="gx-d-flex" style={{gap : '10px'}}>
                        ...
                        <UploadCSV type="getAllSkills" url="bulkImportSkills" table="Exercise"/>
                        <AddNewExercise />
                    </div>}
            >
                <ExerciseTable apiName="getExercises"  scroll={true} pagination={true} columns={columns} />
            </Widget>
            <EditModal/>
        </Layout>
    )
